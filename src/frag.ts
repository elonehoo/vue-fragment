...
const isFrag = (node: any) => 'frag' in node

function patchParentNode(node: any, fakeParent: any) {
...
function patchNextSibling(node: any) {
  ...
  Object.defineProperty(
    ...
    {
      get() {
        const { childNodes }: any = this.parentNode
...
function getTopFragment(node: any, fromParent: any) {
...
let getChildNodes: any

// For a parent to get fake children
function getChildNodesWithFragments(node: any) {
  // In case SSR
  if (!getChildNodes) {
...
function patchChildNodes(node: any) {
...
// FlatMap polyfill
const getFragmentLeafNodes = (children: any) => Array.prototype.concat(
...
function removePlaceholder(node: any) {
  const placeholder = node[$placeholder]
